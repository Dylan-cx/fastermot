...
class PID_posi_2:
    """位置式实现2
    """
    def __init__(self, k=[1., 0., 0.], target = 2, upper=1.0, lower=-1.0):
    ...
    def cal_output(self, error):   # calculate output
        self.reset()
        self.e = error - self.target
        # print('e:{}'.format(error))
        u = float(self.e * self.kp + self.sum_e * \
